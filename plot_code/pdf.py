...
def test_set_percentile(tags_test):
    ...
    return z, a_b, b_c, c_d, d_e, e_, a,b,c,d,e,h_
...
p_0, p_0_20, p_20_40, p_40_60, p_60_80, p_80,a,b,c,d,e,h_ = test_set_percentile(tags_test)
z = np.loadtxt('../z.txt')
...
p_half = h_.reshape(137,1,33,33)

wqv_p_0 = p_0 * y_test*2.5*10**6
wqv_p_0_20 = p_0_20 * y_test*2.5*10**6
wqv_p_20_40 = p_20_40 * y_test*2.5*10**6
wqv_p_40_60 = p_40_60 * y_test*2.5*10**6
wqv_p_60_80 = p_60_80 * y_test*2.5*10**6
wqv_p_80 = p_80 * y_test*2.5*10**6
wqv_p_half = p_half*y_test*2.5*10**6
...
mean_wqv_p_half = np.mean(np.mean(np.mean(wqv_p_half,axis=-1),axis=-1),axis=0)
std_wqv_p_half = np.std(np.std(np.std(wqv_p_half,axis=-1),axis=-1),axis=0)
std_wqv_p_half = np.sqrt(std_wqv_p_half**2 * 137*33*33/p_half[p_half==1].shape[0])
# plot
# 0 percentile
...
plt.savefig('./percentile/0.png',dpi=300)
plt.close()
...
plt.ylabel('Height[m]', fontsize=12)
plt.xlabel(r"$\overline{w'h'}$[J*m/s*kg]", fontsize=12)
plt.savefig('./percentile/0~20.png',dpi=300)
plt.close()
...
plt.ylabel('Height[m]', fontsize=12)
plt.xlabel(r"$\overline{w'h'}$[J*m/s*kg]", fontsize=12)
plt.savefig('./percentile/20~40.png',dpi=300)
plt.close()
...
plt.ylabel('Height[m]', fontsize=12)
plt.xlabel(r"$\overline{w'h'}$[J*m/s*kg]", fontsize=12)
plt.savefig('./percentile/40~60.png',dpi=300)
plt.close()
...
plt.ylabel('Height[m]', fontsize=12)
plt.xlabel(r"$\overline{w'h'}$[J*m/s*kg]", fontsize=12)
plt.savefig('./percentile/60~80.png',dpi=300)
plt.close()
...
plt.ylabel('Height[m]', fontsize=12)
plt.xlabel(r"$\overline{w'h'}$[J*m/s*kg]",fontsize=12)
plt.savefig('./percentile/80~.png',dpi=300)
plt.close()

# 80~ percentile
plt.figure(figsize=(6,6))
plt.title('Verticla moisture eddy flux profile when sigma > 0.5', fontsize=12)
plt.text(-800,15000,'sigma > 0.5')
plt.plot(mean_wqv_p_half, z, linewidth=2.2, label='mean')
plt.fill_betweenx(z,mean_wqv_p_half+std_wqv_p_half, mean_wqv_p_half-std_wqv_p_half, alpha=0.4, label='std')
plt.grid(True)
plt.xlim(-801, 801)
plt.legend(prop={'size': 14})
plt.ylabel('Height[m]', fontsize=12)
plt.xlabel(r"$\overline{w'h'}$[J*m/s*kg]",fontsize=12)
plt.savefig('./percentile/half.png',dpi=300)
plt.close()
# all mean
plt.figure(figsize=(6,6))
...
#plt.plot(mean_wqv_p_half, z, linewidth=2.2, label='sigma > 0.5')
plt.grid(True)
plt.xlim(0, 150)
plt.legend(prop={'size': 14})
plt.ylabel('Height[m]', fontsize=12)
plt.xlabel(r"$\overline{w'h'}$[J*m/s*kg]",fontsize=12)
plt.savefig('./percentile/all_mean.png',dpi=300)
plt.close()
